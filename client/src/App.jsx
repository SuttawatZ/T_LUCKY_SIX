...
function TicketCard({ ticket, onAdd, selected, drawLabel }) {
  return <article className="group overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-[0_8px_24px_rgba(15,23,42,.08)] transition duration-300 hover:-translate-y-1 hover:border-red-200 hover:shadow-[0_14px_32px_rgba(220,38,38,.16)] sm:p-4">
    <div className="flex items-center justify-between gap-2 text-[10px] font-bold">
      <span className="rounded-full bg-[#1e1e1e] px-2.5 py-1.5 text-white">งวด {drawLabel}</span>
      <span className="rounded-full bg-[#dc2626] px-2.5 py-1.5 text-white">{ticket.set}</span>
    </div>
...
export default function App() {
  ...
  const shown = useMemo(() => {
    ...
    return inventory.filter((ticket) => ticket.number.includes(query));
